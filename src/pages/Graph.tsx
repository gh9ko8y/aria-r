...
import type { GraphNode, GraphEdge, GraphMode, GraphFilters, NodeType } from './graph/types';
import {
  mockBooks,
  mockCharacters,
  mockTags,
...
export default function Graph() {
  ...
  const { nodes, edges } = useMemo(() => {
    ...
    if (mode === 'character') {
      ...
      // Character nodes for this book
      const bookChars = mockCharacters.filter((c) => c.bookId === centerBook.id);
      ...
      bookChars.forEach((char) => {
        (char.relations || []).forEach((rel) => {
          const targetChar = bookChars.find((c) => c.id === rel.characterId);
          if (targetChar) {
            const edgeId = `edge-char-${char.id}-${rel.characterId}`;
            if (!graphEdges.find((e) => e.id === edgeId)) {
              graphEdges.push({
                id: edgeId,
                source: char.id,
      ...
      // Character nodes (major characters only, limited)
      const majorChars = mockCharacters.filter((c) => c.importance === 'major').slice(0, 12);
      ...
      for (let i = 0; i < mockBooks.length; i++) {
        for (let j = i + 1; j < mockBooks.length; j++) {
          const b1 = mockBooks[i];
          const b2 = mockBooks[j];
  ...
  const hasData = mockBooks.length > 0;
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: COLORS.bgCream }}>
      {/* Header */}
      <motion.div
        className="px-6 pt-6 pb-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"
