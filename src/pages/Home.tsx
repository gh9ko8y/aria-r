...
import { getCurrentlyReading, getRecentExcerpts, getReadingStats, getGreeting } from '@/lib/storage'
...
export default function Home() {
  ...
  const [greeting, setGreeting] = useState({ text: '', subtext: '' })
  ...
  return (
    <div className="space-y-8">
      <section>
        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-display-xl text-[var(--text-primary)] mb-2">
          {greeting.text || '今天，读了什么？'}
        </motion.h1>
        <p className="text-body-md text-[var(--text-secondary)]">
          {greeting.subtext || new Date().toLocaleDateString('zh-CN', { year: 'numeric', month: 'long', day: 'numeric', weekday: 'long' })}
        </p>
      </section>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.05 }} className="card-surface p-4">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {currentlyReading.map((book, i) => (
              <motion.div key={book.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + i * 0.1 }} className="card-surface p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-2">
                      <div className="h-1 bg-[var(--border-subtle)] rounded-full overflow-hidden">
                        <div className="h-full rounded-full transition-all" style={{ width: `${book.progress}%`, background: 'var(--accent-morandi)' }} />
                      </div>
        ...
        <section>
          ...
          <div className="space-y-3">
            {recentExcerpts.map((excerpt, i) => (
              <motion.div key={excerpt.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + i * 0.1 }} className="excerpt-card">
                <p className="text-body-lg italic text-[var(--text-primary)]">「{excerpt.content}」</p>
                {excerpt.thought && <p className="text-body-md text-[var(--text-secondary)] mt-2">{excerpt.thought}</p>}
              </motion.div>
            ))}
          </div>
        </section>
